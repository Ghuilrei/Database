...
public class ReturnBook extends AppCompatActivity implements View.OnClickListener {
        ...
        if(view.getId() == R.id.bt_com){
            try {
                data = "user_id="+ URLEncoder.encode(person.getUser_id(),"UTF-8")+
                        ...
                        "&return_id="+URLEncoder.encode(et_borrow_id.getText().toString(),"UTF-8");
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }

            // 新建连接线程
            new Thread(new ReturnBook.MyThread()).start();
        }
    }
    ...
    public class MyThread implements Runnable {
        ...
        public void run() {
            //获取服务器返回的数据
            infoString = WebServicePost.executeHttpPost(data, "ReturnBook");
        ...
        private void showResponse(final String response) {
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    // 注册成功
                    if ("B12A0300A0500A1700".equals(recode)) {
                        Toast.makeText(getApplicationContext(), "还书成功", Toast.LENGTH_SHORT).show();
                        finish();
                    } else {
                        Toast.makeText(getApplicationContext(), "还书失败", Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }
